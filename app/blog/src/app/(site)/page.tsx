...
export default async function Home() {
  ...
  const displayedPosts = posts.slice(0, 10); // Limit to a maximum of 10 posts
    ...
    <div>
      ...
      <div className="mt-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {displayedPosts.map((post) => (
          <Link href={`/posts/${post.seoData.slug}`} key={post._id} className="border-2 border-gray-500 rounded-lg p-1 hover:scale-105 hover:border-blue-800 transition">
            {post.contents.cover && (
              <Image
